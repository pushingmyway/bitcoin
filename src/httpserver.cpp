...
static void http_request_cb(struct evhttp_request *req, void *arg) {
  ...
  LogPrint(BCLog::HTTP, "Received a %s request for %s from %s\n",
           RequestMethodString(hreq->GetRequestMethod()), SanitizeString(hreq->GetURI(), SAFE_CHARS_URI).substr(0, 100),
           hreq->GetPeer().ToString());
...
bool InitHTTPServer() {
...
#else
  evthread_use_pthreads();//如果设置过了evthread_use_pthreads,那么bufferevent和从bufferevent取到的evbuffer都会变成线程安全  否则bufferevent和evbuffer是不会保证线程安全的,切记
#endif
  ...
  evhttp_set_gencb(http, http_request_cb, nullptr);//用evhttp_set_gencb设置了http请求的处理函数
