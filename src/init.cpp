...
static boost::thread_group threadGroup; //文件中定义的static静态声明是c的做法，c++是放到未命名的命名空间中的，拥有静态生命周期，程序结束后才销毁
...
void SetupServerArgs() {
  ...
  gArgs.AddArg("-help-debug", "Print help message with debugging options and exit", false,
               OptionsCategory::DEBUG_TEST); // server-only for now
  ...
  gArgs.AddArg("-dbbatchsize",
               ...
               OptionsCategory::OPTIONS);
  gArgs.AddArg("-dbcache=<n>", strprintf(
    "Maximum database cache size <n> MiB (%d to %d, default: %d). In addition, unused mempool memory is shared for this cache (see -maxmempool).",
    nMinDbCache, nMaxDbCache, nDefaultDbCache), false, OptionsCategory::OPTIONS);
  ...
  gArgs.AddArg("-feefilter", strprintf("Tell other nodes to filter invs to us by our mempool min fee (default: %u)",
                                       DEFAULT_FEEFILTER), true, OptionsCategory::OPTIONS);
  ...
  gArgs.AddArg("-maxmempool=<n>", strprintf("Keep the transaction memory pool below <n> megabytes (default: %u)",
                                            DEFAULT_MAX_MEMPOOL_SIZE), false, OptionsCategory::OPTIONS);
  ...
  gArgs.AddArg("-minimumchainwork=<hex>",
               strprintf("Minimum work assumed to exist on a valid chain in hex (default: %s, testnet: %s)",
                         ...
                         testnetChainParams->GetConsensus().nMinimumChainWork.GetHex()), true,
               OptionsCategory::OPTIONS);
  ...
  gArgs.AddArg("-persistmempool", strprintf("Whether to save the mempool on shutdown and load on restart (default: %u)",
                                            DEFAULT_PERSIST_MEMPOOL), false, OptionsCategory::OPTIONS);
  gArgs.AddArg("-pid=<file>", strprintf(
    ...
    BITCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
  gArgs.AddArg("-prune=<n>", strprintf(
    ...
    MIN_DISK_SPACE_FOR_BLOCK_FILES / 1024 / 1024), false, OptionsCategory::OPTIONS);
  ...
  gArgs.AddArg("-banscore=<n>",
               ...
               false, OptionsCategory::CONNECTION);
  gArgs.AddArg("-bantime=<n>", strprintf("Number of seconds to keep misbehaving peers from reconnecting (default: %u)",
  ...
  gArgs.AddArg("-connect=<ip>",
               ...
               false, OptionsCategory::CONNECTION);
  gArgs.AddArg("-discover", "Discover own IP addresses (default: 1 when listening and no -externalip or -proxy)", false,
  ...
  gArgs.AddArg("-listen", "Accept connections from outside (default: 1 if no -proxy or -connect)", false,
               OptionsCategory::CONNECTION);
  gArgs.AddArg("-listenonion", strprintf("Automatically create Tor hidden service (default: %d)", DEFAULT_LISTEN_ONION),
               false, OptionsCategory::CONNECTION);
  gArgs.AddArg("-maxconnections=<n>",
               strprintf("Maintain at most <n> connections to peers (default: %u)", DEFAULT_MAX_PEER_CONNECTIONS),
               false, OptionsCategory::CONNECTION);
  gArgs.AddArg("-maxreceivebuffer=<n>", strprintf("Maximum per-connection receive buffer, <n>*1000 bytes (default: %u)",
                                                  DEFAULT_MAXRECEIVEBUFFER), false, OptionsCategory::CONNECTION);
  ...
  gArgs.AddArg("-timeout=<n>", strprintf("Specify connection timeout in milliseconds (minimum: 1, default: %d)",
                                         DEFAULT_CONNECT_TIMEOUT), false, OptionsCategory::CONNECTION);
  gArgs.AddArg("-peertimeout=<n>", strprintf(
    ...
    DEFAULT_PEER_CONNECT_TIMEOUT), true, OptionsCategory::CONNECTION);
  ...
  gArgs.AddArg("-checklevel=<n>", strprintf("How thorough the block verification of -checkblocks is: "
                                              ...
                                              "(0-4, default: %u)", DEFAULT_CHECKLEVEL), true,
               OptionsCategory::DEBUG_TEST);
  ...
  gArgs.AddArg("-checkmempool=<n>", strprintf("Run checks every <n> transactions (default: %u, regtest: %u)",
                                              ...
                                              regtestChainParams->DefaultConsistencyChecks()), true,
               OptionsCategory::DEBUG_TEST);
  gArgs.AddArg("-checkpoints", strprintf("Disable expensive verification for known chain history (default: %u)",
                                         DEFAULT_CHECKPOINTS_ENABLED), true, OptionsCategory::DEBUG_TEST);
  ...
  gArgs.AddArg("-stopatheight",
               strprintf("Stop running after reaching the given height in the main chain (default: %u)",
                         DEFAULT_STOPATHEIGHT), true, OptionsCategory::DEBUG_TEST);
  ...
  gArgs.AddArg("-debug=<category>",
               ...
               ListLogCategories() + ".", false, OptionsCategory::DEBUG_TEST);
  ...
  gArgs.AddArg("-logips", strprintf("Include IP addresses in debug output (default: %u)", DEFAULT_LOGIPS), false,
               OptionsCategory::DEBUG_TEST);
  gArgs.AddArg("-logtimestamps", strprintf("Prepend debug output with timestamp (default: %u)", DEFAULT_LOGTIMESTAMPS),
               false, OptionsCategory::DEBUG_TEST);
  ...
  gArgs.AddArg("-mempoolreplacement",
               strprintf("Enable transaction replacement in the memory pool (default: %u)", DEFAULT_ENABLE_REPLACEMENT),
  ...
  gArgs.AddArg("-blockmintxfee=<amt>", strprintf(
    "Set lowest fee rate (in %s/kB) for transactions to be included in block creation. (default: %s)", CURRENCY_UNIT,
    FormatMoney(DEFAULT_BLOCK_MIN_TX_FEE)), false, OptionsCategory::BLOCK_CREATION);
  ...
  gArgs.AddArg("-rpcthreads=<n>",
               strprintf("Set the number of threads to service RPC calls (default: %d)", DEFAULT_HTTP_THREADS), false,
               OptionsCategory::RPC);
...
void InitParameterInteraction() {
  ...
  for (const auto &arg : gArgs.GetUnsuitableSectionOnlyArgs()) {
    InitWarning(
      strprintf(_("Config setting for %s only applied on %s network when in [%s] section."), arg, network, network));
  }

  // Warn if unrecognized section name are present in the config file.
  for (const auto &section : gArgs.GetUnrecognizedSections()) {
    InitWarning(
      strprintf("%s:%i " + _("Section [%s] is not recognized."), section.m_file, section.m_line, section.m_name));
  }
}
...
bool AppInitParameterInteraction() {
  ...
  if (nScriptCheckThreads <= 1)
    ...
    nScriptCheckThreads = MAX_SCRIPTCHECK_THREADS;  //脚本检查线程一般是逻辑核的个数，最大是16个
...
bool AppInitMain(InitInterfaces &interfaces) {
  ...
  if (!CreatePidFile()) { //indian 创建pid，在参数-pid指定的路径下 默认是bitcoind.pid
  ...
  if (nScriptCheckThreads) {
    for (int i = 0; i < nScriptCheckThreads - 1; i++)
      threadGroup.create_thread(&ThreadScriptCheck); //thread_group用于管理一组线程，就像是一个线程池，它内部使用std::list  用于脚本检查
  }

  // Start the lightweight task scheduler thread
  CScheduler::Function serviceLoop = std::bind(&CScheduler::serviceQueue, &scheduler);
  threadGroup.create_thread(
    std::bind(&TraceThread<CScheduler::Function>, "scheduler", serviceLoop));  //启动serviceLoop  会从taskQueue取可调用对象进行处理

  GetMainSignals().RegisterBackgroundSignalScheduler(scheduler); //后台scheduler注册回调函数
  ...
  for (const auto &client : interfaces.chain_clients) {  //这里应该没有chain_clients
    client->registerRpcs();
  }
  ...
  for (const auto &client : interfaces.chain_clients) {  //todo no chain_clients
    if (!client->verify()) {
      return false;
    }
  }
  ...
  connOptions.nLocalServices = nLocalServices;  //indian set cnode的service bit NODE_NETWORK表示full node do everything
  connOptions.nMaxConnections = nMaxConnections;
  connOptions.nMaxOutbound = std::min(MAX_OUTBOUND_CONNECTIONS,
                                      connOptions.nMaxConnections);  // indian  最大的outbound = 8
  connOptions.nMaxAddnode = MAX_ADDNODE_CONNECTIONS; //indian 8
  ...
  if (!g_connman->Start(scheduler, connOptions)) {  // indian
    return false;
  }
