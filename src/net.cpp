...
CNode *CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, bool manual_connection) {
  ...
  if (addrConnect.IsValid()) {
    ...
    if (GetProxy(addrConnect.GetNetwork(), proxy)) {
      ...
      connected = ConnectThroughProxy(proxy, addrConnect.ToStringIP(), addrConnect.GetPort(), hSocket, nConnectTimeout,
...
bool CConnman::AttemptToEvictConnection() {
  ...
  if (std::any_of(vEvictionCandidates.begin(), vEvictionCandidates.end(),
...
int CConnman::GetExtraOutboundCount() {
  int nOutbound = 0;
  {
    LOCK(cs_vNodes);
    for (const CNode *pnode : vNodes) {
      if (!pnode->fInbound && !pnode->m_manual_connection && !pnode->fFeeler && !pnode->fDisconnect &&
          !pnode->fOneShot && pnode->fSuccessfullyConnected) {
        ++nOutbound;
      }
    }
  }
...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect) {
  ...
  while (!interruptNet) {
    ...
    if (addrConnect.IsValid()) {
      ...
      OpenNetworkConnection(addrConnect, (int) setConnected.size() >= std::min(nMaxConnections - 1, 2), &grant, nullptr,
                            false, fFeeler);
    }
  }
}
...
std::vector<AddedNodeInfo> CConnman::GetAddedNodeInfo() {
  ...
  std::map<std::string, std::pair<bool, CService>> mapConnectedByName;
  {
    ...
    for (const CNode *pnode : vNodes) {
      ...
      if (!addrName.empty()) {
        mapConnectedByName[std::move(addrName)] = std::make_pair(pnode->fInbound,
...
void CConnman::OpenNetworkConnection(const CAddress &addrConnect, bool fCountFailure, CSemaphoreGrant *grantOutbound,
                                     const char *pszDest, bool fOneShot, bool fFeeler, bool manual_connection) {
  //
...
bool CConnman::Start(CScheduler &scheduler, const Options &connOptions) {
  ...
  for (const auto &strDest : connOptions.vSeedNodes) {  //indian 默认没有
    AddOneShot(strDest);
  }
  ...
  if (semOutbound == nullptr) {
    ...
    semOutbound = MakeUnique<CSemaphore>(std::min((nMaxOutbound + nMaxFeeler), nMaxConnections)); // 8 + 1 == 9
  }
...
CNode::CNode(NodeId idIn, ServiceFlags nLocalServicesIn, int nMyStartingHeightIn, SOCKET hSocketIn,
             const CAddress &addrIn, uint64_t nKeyedNetGroupIn, uint64_t nLocalHostNonceIn, const CAddress &addrBindIn,
...
int64_t PoissonNextSend(int64_t now, int average_interval_seconds) {
  return now + (int64_t) (
    log1p(GetRand(1ULL << 48) * -0.0000000000000035527136788 /* -1/2^48 */) * average_interval_seconds * -1000000.0 +
    0.5);
}
